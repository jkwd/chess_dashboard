...
, final as (
    ...
    *
    ...
    , case
        when time_class = 'daily' then 'daily ' || (time_control_base / 3600 / 24)::int::varchar || ' days'
    ...
    -- GAME TIME DETAILS
    , replace(regexp_extract(pgn, '(UTCDate )"(.*)"', 2), '.', '-')::date as game_start_date
    , regexp_extract(pgn, '(StartTime )"(.*)"', 2) as game_start_time
    , concat(game_start_date, ' ', game_start_time)::timestamp as game_start_timestamp
